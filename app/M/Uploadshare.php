...
class M_Uploadshare extends M_Model
{
    static $db;
    static $document_table = 'fs_share_tree';
    ...
    */
    public static function upload($login_user_info) {
        ...
        if (!empty($login_user_info) && isset($_FILES["uploads"]) || is_uploaded_file($_FILES["uploads"]["tmp_name"]) || $_FILES["uploads"]["error"] != 0) {
            ...
            $savepath = !empty($_REQUEST['savePath']) ? PROJECT_DOC_PATH.M_Sharedocument::splitdocpath($_REQUEST['savePath']) : '';  //路径—+ 文件编号
            if(!$savepath){
            ...
            $verify = M_Usergroup::verify($login_user_info, 'uploadfile', $fs_parent, 'share');
            if(!$verify){
                $rs['msg'] = '对不起，您没有此操作权限';
                $rs['success'] = false;
            ...
            if(!empty($parentDoc['fs_code'])){
                ...
                if($parentDoc['fs_id']!='1'){
                    $log_save_path = substr(M_Sharedocument::getFilenamepath($fs_parent),1);
                    $fs_code = $log_save_path . '-' . $fs_name;
                }else{
                    $log_save_path = $parentDoc['fs_name'];
                    $fs_code = $fs_name;
                }
            } 
            ...
            if($checkresult['flag']==1){
                if(isset($_REQUEST['coverfile'])&& $_REQUEST['coverfile']==1){ //判断用户是否对现有文件允许备份， 不允许的返回错误
                    ...
                    $oldfilename = $checkresult['data']['fs_hashname'] . '.' . $checkresult['data']['fs_type'];
                    ...
                    if(ZF_Libs_IOFile::backup($oldfile, $oldfilename)){
                        ...
                        if($rs){
                            ...
                            //M_Log::doclog($doclog);
                            #记录系统操作日志
                            M_Log::systemlog(array('login_user_name'=>$login_user_info['u_name'], 'login_user_email'=>$login_user_info['u_email'], 'desc'=>'更新目录 '.$log_save_path.' 下的文件 '.substr(M_Sharedocument::getFilenamepath($checkresult['data']['fs_id']), 1).'（'.$checkresult['data']['fs_intro'].'） 为 '.$log_save_path.'-'.$fs_name.'（'.$fs_intro.'） 操作成功！'));
                            header("HTTP/1.1 200 OK");
                            $sql = "select * from " . self::$document_table . " where fs_id='{$checkresult['data']['fs_id']}' ";
                            $return = self::$db->get_row($sql);
                            if($return){
                                $return['fs_fullpath'] =  M_Sharedocument::getParentpath($checkresult['data']['fs_id']);
                                $return['exists'] = 1;
            ...
            if($rs){
                ...
                $return = self::$db->get_row($sql);
                if($return){
                    $return['fs_fullpath'] = M_Sharedocument::getParentpath($log_fs_id);
                    $return['exists'] = 0;
                    echo json_encode($return);
                }else{
                    echo '';
