...
class DB_Sql {
	...
	function query($Query_String) {
        ...
        if (!mysql_select_db($this->Database, $$LinkName)) {
	...
	function get_row($query , $oMethod = 'A'){
		...
		if($oMethod=='A'){ //返回数组
			if($t = @mysql_fetch_assoc($this->Query_ID)){
				$Results=$t;
				$this->Row += 1;
			}
	...
	function get_results($query , $oMethod = 'A', $fetchType = MYSQL_ASSOC){
		$this->query($query); //执行SQL
		...
		if($oMethod=='A'){ //返回二维数组
			while($t = @mysql_fetch_array($this->Query_ID, $fetchType)){
				$Results[]=$t;
				$this->Row += 1;
			}
    ...
    function get_col($query){
        ...
        while($t = @mysql_fetch_array($this->Query_ID,MYSQL_NUM)){
            $Results[]=$t[0];
            $this->Row += 1;
        }
        $this->Errno = mysql_errno();
